...
def fcalcularIMC():
    ...
    alturaT = e_altura.get()
    pesoT = e_peso.get()
    altura = float(alturaT)
    peso = int(pesoT)

    print(peso, altura)

    imcSr = peso / (altura * altura)
    imc = round(imcSr, 2)


    if imc < 18.5:
        result_caixa = Label(frame_baixo, text=imc, height=1, anchor=NW, font=('Ivy 30'),bg=coRamarelo2, fg=coRpreto)
        result_caixa.place(x=270, y=55)
        l_resultado.config(text='abaix do peso')
        ...
        result_caixa = Label(frame_baixo, text=imc, height=1, anchor=NW, font=('Ivy 30'),bg=coRamarelo2, fg=coRpreto)
        result_caixa.place(x=270, y=55)
        l_resultado = Label(frame_baixo, text="sobrepeso", height=1, anchor=NW, font=("Ivy 12 bold"),justify='center', bg=coRcinza, fg=coRpreto, highlightbackground=coRpreto, relief='solid')
        l_resultado.place(x=250,y=120)
    else:
        result_caixa = Label(frame_baixo, text=imc, height=1, anchor=NW, font=('Ivy 30'),bg=coRamarelo2, fg=coRpreto)
        result_caixa.place(x=270, y=55)
        l_resultado = Label(frame_baixo, text="obesidade", height=1, anchor=NW, font=("Ivy 12 bold"),justify='center', bg=coRcinza, fg=coRpreto, highlightbackground=coRpreto, relief='solid')
        l_resultado.place(x=250,y=120)
...
l_altura = Label(frame_baixo, text="altura:", height=1, anchor=NW, font=("Ivy 12 bold"), bg=coRcinza, fg=coRbranco)
...
l_peso = Label(frame_baixo, text="peso:", height=1, anchor=NW, font=("Ivy 12 bold"), bg=coRcinza, fg=coRbranco)
